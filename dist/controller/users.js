...
const smsCodeExp = utils_1.num2Time(expMins);
...
const smsCodeModel = mongoose_1.default.model('smscodes', smsCodeSchema);
module.exports = {
    userModel,
    smsCodeModel,
    async sendSmsCode(ctx) {
        ...
        if (!phone)
            throw utils_1.useError('请输入手机号', 401, false);
        const code = utils_1.getRandomNumber(4);
    ...
    async remove(ctx) {
        ...
        if (!one)
            throw utils_1.useError('账号不存在', 401, false);
        one.removed = true;
        await one.save();
    },
    async update(ctx) {
    ...
    async login(ctx) {
        ...
        if (type === 'I') { // smscode
            ...
            if (!one)
                throw utils_1.useError('没有下发验证码', 401, false);
            if (code !== one.code)
                throw utils_1.useError('请输入正确的验证码', 401, false);
            const now = +new Date();
            const exp = +new Date(one.expiresIn);
            if (now >= exp)
                throw utils_1.useError('验证码已过期', 401, false);
            // 验证通过
            ...
            if (!pwd)
                throw utils_1.useError('请输入密码', 401, false);
            item = await userModel.findOne(query).lean();
            // if (!item) {
            //   throw useError('用户不存在', 401, false)
            // } else if (item.pwd !== pwd) throw useError('密码错误', 401, false)
            if (!item) {
                throw utils_1.useError('用户不存在', 401, false);
            }
            else {
                if (item.pwd) {
                    if (item.pwd !== pwd)
                        throw utils_1.useError('密码错误', 401, false);
                }
                else {
                    // 初始密码 111111
                    if ('111111' !== pwd)
        ...
        const token = jwt_1.generateToken({ _id: item._id });
        const userInfo = {
            name: item.name,
            phone: item.phone,
            gender: item.gender,
